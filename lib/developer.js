              ...
              db.query(`SELECT SUM(count) AS todayTotal FROM count WHERE year=${year} AND month=${month} AND day=${day}`, function(error6, todayTotal){ if(error6){throw error6;}
                db.query(`SELECT SUM(count) AS todayAdmin FROM count WHERE year=${year} AND month=${month} AND day=${day} AND id='admin'`, function(error7, todayAdmin){ if(error7){throw error7;}
                  db.query(`SELECT SUM(count) AS todayUser FROM count WHERE year=${year} AND month=${month} AND day=${day} AND id='user'`, function(error8, todayUser){ if(error8){throw error8;}
                    db.query(`SELECT * FROM team_info ORDER BY CAST(admin_id AS SIGNED)`, function(error9, team_info){  if(error9){throw error9;}
                      db.query(`SELECT A.admin_id, COUNT(*) activityNum FROM team_info A LEFT OUTER JOIN activity B ON A.admin_id = B.admin_id GROUP BY A.admin_id, B.admin_id ORDER BY CAST(A.admin_id AS SIGNED)`, function(error10, activityNum){  if(error10){throw error10;}
                        db.query(`SELECT A.admin_id, COUNT(*) imgNum FROM team_info A LEFT OUTER JOIN cc B ON A.admin_id = B.admin_id GROUP BY A.admin_id, B.admin_id ORDER BY CAST(A.admin_id AS SIGNED)`, function(error11, imgNum){  if(error11){throw error11;}
                            ...
                            var activityCount = []; // 팀별 activity 수 리스트
                            for(var i = 0 ; i < team_info.length ; i++){
                                if(activityNum[i] === undefined){
                                    activityCount[i] = 0;
                                } else {
                                    activityCount[i] = activityNum[i].activityNum;
                                }
                            }

                            var teamImgCount = []; // 팀별 이미지 수 리스트
                            for(var i = 0 ; i < team_info.length ; i++){
                                if(imgNum[i] === undefined){
                                    teamImgCount[i] = 0;
                                } else{
                                    teamImgCount[i] = imgNum[i].imgNum;
                                }
                            }
                
                            var html = template_developer.HTML_developerMain(activityCount, teamImgCount, team_info, alertFlag, totalViewers, adminViewers, userViewers, imgCount, teamNum, todayTotalViewers, todayAdminViewers, todayUserViewers);
                            response.writeHead(200);
                            response.end(html);
                        });     
                      });    
