...
exports.main = function(request, response, random_url){
  db.query(`SELECT * FROM url WHERE url = ?`, [random_url], function(error1, url){ if(error1){ throw error1; }
    if(url.length < 1){
	  ...
	  db.query(`SELECT * FROM count WHERE id='user' ORDER BY CAST(year AS int) DESC, CAST(month AS int) DESC,CAST(day AS int) DESC`,function(error2,count){ if(error2){ throw error2; }
