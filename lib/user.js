              ...
              var body = `
                ...
                <div class="divCSS">
                  ...
                  <p class="totalScore"> Total Score : ${total(sum_ccac)} </p>
		            ...
		            ${state_tag} <br/>
                ${template_user.function_activityTable(sum_ccac ,random_url,activity,url[0].team_num,team_info)} <br/>
              `;
  ...
  request.on('end', function(){
    ...
    db.query(`SELECT * FROM url WHERE url="${random_url}"`,function(error1,url){ if(error1){ throw error1; }
      db.query(`DELETE FROM cc WHERE activity_id=? AND admin_id=? AND activity_index=?`, [post.activity_id,url[0].admin_id, post.actindex], function(error2, result){ if(error2){ throw error2; }
            console.log(post.activity_id,url[0].admin_id, post.actindex);
  ...
  db.query(`SELECT * FROM url WHERE url=?`,[rand_url],function(error1,admin_id){ if(error1){ throw error1; }
    db.query(`INSERT INTO cc VALUE('${admin_id[0].admin_id}','${request.body.team_num}','${request.body.actid}','${base64image}','${request.file.size}','${request.file.mimetype}','${request.body.actindex}')`, function(error2,cc){
      if(error2){
        ...
        `;
        var html = template_user.HTML_main(alert, '', '');
        response.writeHead(200);
        response.end(html);
      }
      else{
