...
exports.main = function(request, response, random_url){
  db.query(`SELECT * FROM url WHERE url = ?`, [random_url], function(error1, url){ if(error1){ throw error1; }
    if(url.length < 1){
	  ...
	  db.query(`SELECT * FROM count WHERE id='user' ORDER BY year DESC,month DESC,day DESC`,function(error2,count){ if(error2){ throw error2; }
      db.query(`UPDATE count SET count=? WHERE id='user' AND year=? AND month=? AND day=?`,[Number(count[0].count)+1,count[0].year, count[0].month, count[0].day],function(error3,state){ if(error3){ throw error3; }
          db.query(`SELECT * FROM activity LEFT JOIN (SELECT * FROM cc WHERE team_num=? AND admin_id=?) AS select_cc ON id=activity_id WHERE activity.admin_id=?;`,[url[0].team_num,url[0].admin_id,url[0].admin_id], function(error4, sum_ccac){ if(error4){ throw error4; }
            db.query(`SELECT id,COUNT(*) AS total, activity_num FROM activity LEFT JOIN (SELECT * FROM cc WHERE team_num=? AND admin_id=?) AS select_cc ON id=activity_id WHERE activity.admin_id=? GROUP BY id;`, [url[0].team_num,url[0].admin_id,url[0].admin_id],function(error5, count_index){ if(error5){ throw error5; }
...
exports.image_upload = function(request, response, rand_url){
  ...
  console.log("in image_upload, actUpload_num: ", request.body.actUpload_num);
  console.log(request.body);

  db.query(`SELECT * FROM url WHERE url=?`,[rand_url],function(error1,admin_id){ if(error1){ throw error1; }
    db.query(`INSERT INTO cc VALUE('${admin_id[0].admin_id}','${request.body.team_num}','${request.body.actid}','${base64image}','${request.file.size}','${request.file.mimetype}','${request.body.actUpload_num}')`, function(error2,cc){
      console.log(error2);
      if(error2){
        var alert = `
          <script>
