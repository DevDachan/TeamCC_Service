...
exports.home = function(request, response, random_url){
              ...
              db.query(`SELECT * FROM CC JOIN activity ON id=activity_id WHERE team_num= ?`,[url[0].team_num], function(error3, sum_ccac){
                ...
                db.query(`SELECT * FROM state`, function(error2, state){
                  if(error2){
                    ...
                    `;
                    var html =
                    template_user.user_HTML(
                    ``,
                    `${body}
                    <form style="text-align:center;margin:auto; margin-top:50px;" action="/user_main" method="post">
...
exports.user_main = function(request, response,random_url){
  var body = '';
  ...
  db.query(`SELECT * FROM state`, function(error,state){
    ...
    db.query(`SELECT * FROM CC LEFT JOIN activity ON activity.id = CC.activity_id ORDER BY team_num,activity_id`,function(error2, cc){
      db.query(`SELECT * FROM activity`, function(error3,activity){
        ...
        var html = template_user.main_HTML(body,state[0].title);
...
exports.user_submit = function(request, response){
    var body = '';
    request.on('data', function(data){
        body = body + data;
    });
    request.on('end', function(){
        var post = qs.parse(body);
        db.query(`SELECT * FROM CC`, function(err2, activity){
            db.query(`INSERT INTO CC (team_num, activity_id, content)
            VALUES(?, ?, ?)`,
            [post.team_num, post.id, '멍멍'], // post.author : template_user에 name과 같은 값
            function(error, result){
              if(error){
                  var alert = `<script>
                  alert('등록 오류!! (한 활동에 한번만 입력하세요 or 잘못 된 data)');
                  location.href = 'http://localhost:3000/user?id=${post.rand_url}';
                  </script>`;
                  var html = template_user.user_HTML(alert, '', '');
                  response.writeHead(200);
                  response.end(html);
                }
                else{

                response.writeHead(302, {Location: `user?id=${post.rand_url}`});
                response.end();
              }
            });
        });
