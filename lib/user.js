...
exports.main = function(request, response, random_url){
  db.query(`SELECT * FROM url WHERE url = ?`, [random_url], function(error1, url){ if(error1){ throw error1; }
    if(url.length < 1){
	  ...
	  db.query(`SELECT * FROM count WHERE id='user' ORDER BY CAST(year AS SIGNED) DESC, CAST(month AS SIGNED) DESC,CAST(day AS SIGNED) DESC`,function(error2,count){ if(error2){ throw error2; }
      db.query(`UPDATE count SET count=? WHERE id='user' AND year=? AND month=? AND day=?`,[Number(count[0].count)+1,count[0].year, count[0].month, count[0].day],function(error3,state){ if(error3){ throw error3; }
          db.query(`SELECT * FROM activity LEFT JOIN (SELECT * FROM cc WHERE team_num=? AND admin_id=?) AS select_cc ON id=activity_id WHERE activity.admin_id=?;`,[url[0].team_num,url[0].admin_id,url[0].admin_id], function(error4, sum_ccac){ if(error4){ throw error4; }
            db.query(`SELECT id,COUNT(case when size IS NOT NULL then 1 end) AS total, activity_num,MAX(activity_index) as max_index FROM activity LEFT JOIN (SELECT * FROM cc WHERE team_num=? AND admin_id=?) AS select_cc ON id=activity_id WHERE activity.admin_id=? GROUP BY id;`, [url[0].team_num,url[0].admin_id,url[0].admin_id],function(error5, count_index){ if(error5){ throw error5; }
                ...
                var body = `
                  ...
                  <div class="divCSS">
                    ...
                    <p class="totalScore"> Total Score : ${total(sum_ccac)} </p>
  		            ...
  		            ${state_tag} <br/>
                  ${template_user.function_activityTable(sum_ccac, random_url, count_index, url[0].team_num, team_info)} <br/>
                `;

                var banner = "";
                if(request.session.uid !== null || request.session.uid !== undefined){
                  banner = `<a class="navbar-brand" href="/teamcc/admin_main" >ADMIN PAGE</a>`;
                }else{
                  banner = `<a class="navbar-brand" href="/teamcc/user?id=${random_url}">USER PAGE</a>`;
                }

                var html = template_user.HTML_main(banner,
                  `
                    <link rel="stylesheet" type="text/css" href="css/user.css" />
                    ${body}
                    <form class="formCSS" action="/teamcc/user_score" method="post">
                      <input type="hidden" name="rand_url" value="${random_url}">
                      <input class="scoreBoardBtn" type="submit" value="Score Board">
                    </form>
                    <form class="formCSS" style="margin-top:10px;" action="/teamcc/image_card" method="get">
                      <input type="hidden" name="team_num" value="${team_info[0].num}">
                      <input type="hidden" name="id" value="${random_url}">
                      <input class="scoreBoardBtn" type="submit" value="Score Card">
                    </form> <br/>
...
exports.image_card = function(request,response){
  ...
  request.on('end', function(){
    ...
    db.query(`SELECT admin_id FROM url WHERE url=?`,[request.query.id], function(error, admin_info){ if(error){throw error;}
          ...
          db.query(`SELECT * FROM (SELECT * FROM color WHERE admin_id=?) AS K WHERE score LIKE('%background%') OR score LIKE('%score_color%') OR score LIKE('%title_color%') ORDER BY CAST(score AS SIGNED)`,[admin_id],function(error4,color_db){  if(error4){throw error4;}
            db.query(`SELECT * FROM color WHERE admin_id=? AND score LIKE('%text%') ORDER BY CAST(score AS SIGNED)`,[admin_id],function(error5, text_color_db){ if(error5){throw error5;}
              db.query(`SELECT * FROM color WHERE admin_id=? AND score NOT LIKE('%text%') AND score NOT LIKE("background") AND score NOT LIKE("%color%") ORDER BY CAST(score AS SIGNED)`,[admin_id],function(error6, score_color_db){ if(error6){throw error6;}
                var html = template_user.HTML_cardimage(activity,team_info[0],color_db,admin_id,text_color_db,score_color_db,url_num);
      ...
      db.query(`SELECT * FROM team_info WHERE admin_id=?`,[url[0].admin_id], function(error2,team_info){ if(error2){ throw error2; }
        db.query(`SELECT DISTINCT cc.admin_id,team_num,activity_id,size,image,mimetype,id,activity,score FROM cc LEFT JOIN activity ON activity.id = cc.activity_id WHERE cc.admin_id='${url[0].admin_id}' AND activity.admin_id='${url[0].admin_id}' ORDER BY CAST(team_num AS SIGNED),CAST(activity_id AS SIGNED)`,function(error3, cc){ if(error3){ throw error3; }
          db.query(`SELECT * FROM activity WHERE admin_id=? ORDER BY CAST(score AS SIGNED)`,[url[0].admin_id], function(error4,activity){ if(error4){ throw error4; }
            ...
            var html = template_user.HTML_score(banner,body,team_info,post.rand_url);
            response.writeHead(200);
            response.end(html);
          });
        });
...
function total(acitivity){
  var sum = 0;
  for(var i = 0; i< acitivity.length; i++){
    if(acitivity[i].size !== null){
      sum += Number(acitivity[i].score);
    }
  }
  return sum;
}
...
exports.activity_delete = function(request, response,random_url){
  ...
  request.on('end', function(){
    var post = qs.parse(body);
    db.query(`SELECT * FROM url WHERE url="${random_url}"`,function(error1,url){ if(error1){ throw error1; }
      db.query(`DELETE FROM cc WHERE activity_id=? AND admin_id=? AND team_num=? AND activity_index=?`, [post.activity_id,url[0].admin_id,post.team_num, post.actindex], function(error2, result){ if(error2){ throw error2; }
...
exports.image_upload = function(request, response, rand_url){
  ...
  if(request.file !== undefined ){
    ...
    db.query(`SELECT * FROM url WHERE url=?`,[rand_url],function(error1,admin_id){ if(error1){ throw error1; }
      db.query(`INSERT INTO cc VALUE('${admin_id[0].admin_id}','${request.body.team_num}','${request.body.actid}','${base64image}','${request.file.size}','${request.file.mimetype}','${request.body.actindex}')`, function(error2,cc){
        if(error2){
          var alert = `
            <script>
              alert('등록 오류!! ${admin_id[0].admin_id} ${request.body.team_num} ${request.body.actid} ${request.body.actindex}');
              location.href = 'http://handongapp.cafe24.com/user?id=${rand_url}';
            </script>
          `;
          var html = template_user.HTML_alert(alert);
          response.writeHead(200);
          response.end(html);
        }
        else{
