    ...
    if(url.length < 1){
      ...
      db.query(`UPDATE count SET count=? WHERE id='user' AND year=? AND month=? AND day=?`,[Number(count[0].count)+1,count[0].year, count[0].month, count[0].day],function(error3,state){ if(error3){ throw error3; }
          db.query(`SELECT * FROM activity LEFT JOIN (SELECT * FROM cc WHERE team_num=? AND admin_id=?) AS select_cc ON id=activity_id WHERE activity.admin_id=?;`,[url[0].team_num,url[0].admin_id,url[0].admin_id], function(error4, sum_ccac){ if(error4){ throw error4; }
            db.query(`SELECT id,COUNT(*), activity_num FROM activity LEFT JOIN (SELECT * FROM cc WHERE team_num=? AND admin_id=?) AS select_cc ON id=activity_id WHERE activity.admin_id=? GROUP BY id;`, [url[0].team_num,url[0].admin_id,url[0].admin_id],function(error5, count_index){ if(error5){ throw error5; }
        ...
        db.query(`SELECT DISTINCT cc.admin_id,team_num,activity_id,size,image,mimetype,id,activity,score FROM cc LEFT JOIN activity ON activity.id = cc.activity_id WHERE cc.admin_id='${url[0].admin_id}' AND activity.admin_id='${url[0].admin_id}' ORDER BY CAST(team_num AS SIGNED),CAST(activity_id AS SIGNED)`,function(error3, cc){ if(error3){ throw error3; }
          db.query(`SELECT * FROM activity WHERE admin_id=? ORDER BY CAST(score AS SIGNED)`,[url[0].admin_id], function(error4,activity){ if(error4){ throw error4; }
            ...
            if(request.session.uid != null){
              ...
              banner = `<a class="navbar-brand" href="/teamcc/user?id=${post.rand_url}">USER PAGE</a>`;
            }
