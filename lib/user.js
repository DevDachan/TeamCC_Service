    ...
    db.query(`SELECT * FROM URL WHERE url = ?`, [random_url], function(error1, url){
      ...
      if(url.length <1){
        ...
        response.end(`
          ...
          db.query(`SELECT * FROM activity WHERE admin_id=?`,[url[0].admin_id], function(error2, activity){
            if(error2){
              throw error2;
            }
...
exports.user_score = function(request, response,random_url){
  ...
  db.query(`SELECT * FROM team_info`, function(error,team_info){
    if(error){
      throw error;
    }
    db.query(`SELECT * FROM CC LEFT JOIN activity ON activity.id = CC.activity_id ORDER BY team_num,activity_id`,function(error2, cc){
      db.query(`SELECT * FROM activity`, function(error3,activity){
        ...
        var html = template_user.score_HTML(body,team_info[0].title);
      ...
      db.query(`SELECT * FROM url WHERE url=?`,[rand_url],function(error,admin_id){
        ...
        db.query(`INSERT INTO cc VALUE('${admin_id[0].admin_id}','${request.body.team_num}','${request.body.actid}','${base64image}','${request.file.size}','${request.file.mimetype}')`, function(error1,cc){
              if(error1){
                  ...
                  location.href = 'http://114.199.252.210:3000/user?id=${rand_url}';
                  </script>`;
