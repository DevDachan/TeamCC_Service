  ...
  urlTable:function(url,team_num){
    ...
    for(var i = 0; i< url.length; i++){
      ...
      <tr>
        ...
        <td>
        <a href="http://114.199.252.210:3000/user?id=${url[i].url}">http://114.199.252.210:3000/user?id=${url[i].url}</td>
        <td>
    ...
    activityImage:function(activity,team_info,color_db){
      ...
      for(var i =0; i<length; i++){
        if(i < activity.length){
          if(score_temp !== activity[i].score){
            ...
            temp +=`
              <td id="td_table">
                <p style="text-align:left; margin-left:15px; margin-bottom:2px; font-size:1.5em;  font-family: 'Jua' ,sans-serif;color:${score_color};">${activity[i].score}학점</p>
      ...
      table = `
      <table id="tb_activityImage" style="background:${background_color}">
      ${table}
      </table>
      `
      return table;
