module.exports = {
                ...
                <form style="  margin:auto;width:400px;" action="/admin_register_state" method="post">
                  <div id="grid">
                    ...
                    <input id="bt_login" type="submit" value="CREATE" class="btn btn-primary btn-xl rounded-pill mt-5" ></input>
			          ...
			          <a href="http://localhost:80/" class="btn btn-primary btn-xl rounded-pill mt-5" style="width:180px;">BACK</a>
              </div>
            </div>
            <div class="bg-circle-1 bg-circle"></div>
            <div class="bg-circle-2 bg-circle"></div>
  ...
  // url table 생성
  urlTable:function(url,team_num,team_title){
    var tag = `
      <tr>
        <th>
          <div class="thCSS" style="background-color: #9EAEFF;"></div>
    ...
    var entireText = `${team_title}\n`;
    for(var i = 0; i < url.length; i++){
      entireText += `${i} Team\n http://localhost:80/user?id=${url[i].url} \n\n`;
    }

    tag +=`
    <tr>
      <td class="tdCSS" colspan="3">
        <div class="thTextCSS">Copy All Links</div>
      </td>
      <td class="tdCSS">
        <button class="bt-copy" onClick="copy_clipboard_text()">
          <img class="img-copy" id="img-copy-entire" src='assets/copy.png' alt='copy'>
          <input type="hidden" id="entire-text" value="${entireText}"></input>
        </button>
      </td>
    </tr>
    `;
      ...
      <html lang="en">
        <head>
          ...
          <script>
            ...
            function copy_clipboard_text(){
              var img = document.getElementById("img-copy-entire");
              img.style.cssText = "filter: opacity(0) drop-shadow(0 0 0 white);";

              var copyText = document.getElementById("entire-text").value;
              const copy_content = document.createElement("textarea");
              document.body.appendChild(copy_content);
              copy_content.value = copyText;
              copy_content.select();
                    ...
                    <div style="display:grid; margin-bottom:20px;   grid-template-rows: 50px 50px; grid-template-columns:2fr 1fr 0.5fr; place-items:centers;">
                      ...
                      <form action="/url_insert" style="grid-row:2; grid-column-start:2; grid-column-end:4; text-align:center;" method="post">
                        ...
                        <input type="submit" style="background-color: #6281D7; border-radius: 20px; vertical-align: middle; box-shadow: 3px 3px 5px grey; border:none;  height:40px; width:200px; color: white; font-size: 15px;" value="Add CCTeam(URL)">
				            ...
				            <table width="100%">
                      ${this.urlTable(url,url_team_num,team_info[0].title)}
                    </table>
                  </div>
                </div>
              </div>
