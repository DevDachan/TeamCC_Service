...
exports.home = function(request, response,queryData){
  db.query(`SELECT * FROM activity`, function(error, activity){
    ...
    db.query(`SELECT * FROM count WHERE id='admin'`,function(error2,count){
    if(error2){
	throw error2;
    }
      db.query(`UPDATE count SET count=? WHERE id='admin'`,[Number(count[0].count)+1],function(error3,state){
       if(error3){
	 throw error3;
       }
