  ...
  request.on('end', function(){
    ...
    if(post.pwd1 !== post.pwd2){
      var err_html = `
        ...
        location.href = 'http://teamcc.cafe24.com:3000/admin_register';
        </script>
        </body>
        </html>`;
      response.writeHead(200);
    ...
    db.query('SELECT * FROM admin WHERE id =? AND password=?',[post.id, post.pwd], function(error, result){
      ...
      db.query('SELECT * FROM admin WHERE id =?', [post.id], function(error2, id_result){
        ...
        if ( result[0]!== undefined){
          ...
          location.href = 'http://teamcc.cafe24.com:3000/';
          </script>
          </body>
          </html>`;
          response.writeHead(200);
...
exports.score = function(request,response){
  ...
  db.query(`SELECT * FROM team_info WHERE admin_id=?`,[request.session.uid] ,function(error,team_info){
    ...
    db.query(`SELECT * FROM CC LEFT JOIN activity ON activity.id = CC.activity_id AND CC.admin_id=? ORDER BY CAST(team_num AS SIGNED),CAST(activity_id AS SIGNED)`,[request.session.uid] ,function(error2, cc){
      if(error2){
        throw error2;
      }
...
exports.create_state = function(request,response){
  ...
  request.on('end', function(){
      ...
      if(team_info.length > 0){
        ...
        location.href = 'http://teamcc.cafe24.com:3000/admin_main';
        </script>
        </body>
        </html>`
        ;
    //    var html = template_admin.alert_HTML(alert);
        ...
        response.end(alert);
          ...
          db.query(`INSERT INTO team_info VALUES(?,?,?,NULL,NULL)`,[request.session.uid, post.team_title+' TEAM CC', post.team_num],function(error2, result){
              if(error2){
                throw error2;
              }
