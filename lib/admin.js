...
exports.activity_image = function(request,response){
  ...
  db.query(`SELECT * FROM activity LEFT JOIN color USING(admin_id,score) WHERE admin_id=? ORDER BY score ASC`,[request.session.uid], function(error2,activity){
      if(error2){
        throw error2;
      }
...
exports.change_color = function(request,response){
  ...
  request.on('end', function(){
      var post = qs.parse(body);
      db.query(`SELECT * FROM color WHERE admin_id=?`,[request.session.uid],function(error, color){
        if(error){
          throw error;
        }
        for(var i = 0; i< color.length; i++){
          db.query(`UPDATE color SET admin_id=? , color='${post['color_'+color[i].score]}' WHERE admin_id =? AND score=?`,[request.session.uid,request.session.uid,color[i].score],function(error,result){
            if(error){
              throw error;
            }
  ...
  request.on('end', function(){
    ...
    //    var html = template_admin.alert_HTML(alert);
        ...
        response.end(alert);
          ...
          db.query(`INSERT INTO team_info VALUES(?,?,?)`,
            [request.session.uid,post.team_title+' TEAM CC', post.team_num],
