      ...
      var err_html = `
        ...
        <html>
          ...
          <body>
            <h1>ERROR</h1>
            <script>
              window.open('http://localhost:80/alert_popup?content=비밀번호가 일치하지 않습니다. 다시 확인해주세요!', '팝업 테스트', 'width=460, height=130, top=10, left=100');
        ...
        if(result[0] !== undefined){
          var err_html = `
            ...
            <html>
              ...
              <body>
                <script>
                  window.open('http://localhost:80/alert_popup?content=이미 존재하는 아이디입니다!', '팝업 테스트', 'width=460, height=130, top=10, left=100');
                  location.href = 'http://localhost:80/admin_register';
                </script>
              </body>
            </html>`;
        ...
        if(result[0]!== undefined){
          ...
          alert = `
            ...
            <html>
              ...
              <body>
                <script>
                  window.open('http://localhost:80/alert_popup?content=비밀번호를 다시 확인해주세요!', '팝업 테스트', 'width=460, height=130, top=10, left=100');
          ...
          alert = `
            ...
            <html>
              ...
              <body>
                <script>
                  window.open('http://localhost:80/alert_popup?content=등록되지 않은 사용자입니다.<br>회원가입 후 이용해주세요!', '팝업 테스트', 'width=460, height=130, top=10, left=100');
                  location.href = 'http://localhost:80/';
                </script>
              </body>
            </html>`;
  ...
  if(request.session.uid === undefined){
    var alert = `
      ...
      <html>
        ...
        <body>
          <script>
            window.open('http://localhost:80/alert_popup?content=로그인 세션 정보가 존재하지 않습니다.<br>로그인 화면으로 돌아갑니다!', '팝업 테스트', 'width=460, height=130, top=10, left=100');
            location.href = 'http://localhost:80/';
          </script>
        </body>
      </html>`;
...
exports.initialization_state = function(request, response){
  var alert = `
    ...
    <html>
      ...
      <body>
        <script>
          window.open('http://localhost:80/confirm_popup?content=모든 팀 CC 활동들을 삭제하시겠습니까?<br>(복구 불가능)', '팝업 테스트', 'width=460, height=200, top=10, left=100');
      ...
      if(team_info.length > 0){
        ...
        <html>
          ...
          <body>
            <h1>ERROR</h1>
            <script>
              window.open('http://localhost:80/alert_popup?content=이미 등록된 팀 정보가 존재합니다!', '팝업 테스트', 'width=460, height=130, top=10, left=100');
              location.href = 'http://localhost:80/admin_main';
            </script>
          </body>
        </html>`;
...
exports.update_state = function(request, response){
  ...
  if(request.file === undefined){
    ...
    db.query(`UPDATE team_info SET title=? , admin_id=?, background_image=?, background_mimetype=? WHERE admin_id =?`, [post.team_title,  post.admin_id, base64image,request.file.mimetype ,request.session.uid ], function(error1, result){  if(error1){throw error1;}
      db.query(`UPDATE admin SET id=? ,password=? WHERE id=?`,[post.admin_id,post.admin_pwd ,request.session.uid],function(error2, result2){  if(error2){throw error2;}
        if(post.admin_id !== request.session.uid){
          db.query(`UPDATE activity SET admin_id=? WHERE admin_id=?`,[post.admin_id ,request.session.uid],function(error3, result3){  if(error3){throw error3;}
            db.query(`UPDATE cc SET admin_id=? WHERE admin_id=?`,[post.admin_id ,request.session.uid],function(error4, result4){  if(error4){throw error4;}
