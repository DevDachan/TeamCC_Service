        ...
        <html>
          ...
          <body>
            ...
            <script>
              ...
              location.href = 'http://localhost:80/admin_register';
            </script>
          </body>
        </html>`;
      response.writeHead(200);
  ...
  if(request.session.uid === undefined){
    var alert = `
      ...
      <html>
        ...
        <body>
          <script>
            alert('세션 정보가 존재하지 않습니다. 로그인 화면으로 돌아갑니다)');
            location.href = 'http://localhost:80/';
          </script>
        </body>
      </html>`;
    ...
    db.query(`SELECT * FROM activity LEFT JOIN color USING(admin_id,score) WHERE admin_id=? AND score <> "background" AND score NOT LIKE("$color%") ORDER BY CAST(score AS SIGNED)`,[request.session.uid], function(error2,activity){ if(error2){throw error2;}
      db.query(`SELECT * FROM team_info WHERE admin_id=?`,[request.session.uid],function(error3, team_info){  if(error3){throw error3;}
        db.query(`SELECT * FROM color WHERE admin_id=? ORDER BY CAST(score AS SIGNED)`,[request.session.uid],function(error4,color_db){  if(error4){throw error4;}
...
exports.change_color = function(request,response){
  ...
  request.on('end', function(){
    var post = qs.parse(body);
    db.query(`SELECT * FROM color WHERE admin_id=? AND score NOT LIKE("background") AND score NOT LIKE("%color%")`,[request.session.uid],function(error, color){  if(error){throw error;}
      db.query(`UPDATE team_info SET title=? WHERE admin_id=? `,[post.new_title,request.session.uid],function(error0,res){  if(error0){throw error0;}
        for(var i = 0; i< color.length; i++){
          db.query(`UPDATE color SET admin_id=? , color='${post['color_'+color[i].score]}' WHERE admin_id =? AND score=?`,[request.session.uid,request.session.uid,color[i].score],function(error2,result){  if(error2){throw error2;}  });
        }
        db.query(`UPDATE color SET color=? WHERE admin_id=? AND score="background"`,[post.background,request.session.uid],function(error3,result2){  if(error3){throw error3;}
          db.query(`UPDATE color SET color=? WHERE admin_id=? AND score="title_color"`,[post.title_color,request.session.uid],function(error4,result2){  if(error4){throw error4;}
              db.query(`UPDATE color SET color=? WHERE admin_id=? AND score="text_color"`,[post.text_color,request.session.uid],function(error5,result2){  if(error5){throw error5;}
                db.query(`UPDATE color SET color=? WHERE admin_id=? AND score="score_color"`,[post.score_color,request.session.uid],function(error6,result2){  if(error6){throw error6;}
        });});});});
...
exports.home = function(request, response,queryData){
  db.query(`SELECT * FROM activity`, function(error, activity){  if(error){throw error;}
    db.query(`SELECT * FROM count WHERE id='admin'`,function(error2,count){  if(error2){throw error2;}
      db.query(`UPDATE count SET count=? WHERE id='admin'`,[Number(count[0].count)+1],function(error3,state){  if(error3){throw error3;}
      ...
      <body>
        <script>
          ...
          if(result === true){
            ...
            location.href = 'http://localhost:80/admin_main';
	        }
        </script>
      </body>
    </html>`;
...
exports.main = function(request,response){
  if(request.session.uid === undefined){
    ...
    db.query(`SELECT * FROM count WHERE id='admin'`,function(error1,count){  if(error1){throw error1;}
      db.query(`UPDATE count SET count=? WHERE id='admin'`,[Number(count[0].count)+1],function(error2,state){  if(error2){throw error2;}
        db.query(`SELECT * FROM team_info WHERE admin_id=?`,[request.session.uid], function(error3,team_info){  if(error3){throw error3;}
        ...
        db.query(`INSERT INTO team_info VALUES(?,?,?,NULL,NULL,"Yes")`,[request.session.uid, post.team_title+' TEAM CC', post.team_num],function(error2, result){  if(error2){throw error2;}
          db.query(`INSERT INTO color VALUES(?,?,?)`,[request.session.uid,"background", "#dda0dd"],function(error3, result){  if(error3){throw error3;}
            db.query(`INSERT INTO color VALUES(?,?,?)`,[request.session.uid,"title_color", "#ffffff"],function(error4, result){  if(error4){throw error4;}
              db.query(`INSERT INTO color VALUES(?,?,?)`,[request.session.uid,"text_color", "#ffffff"],function(error5, result){  if(error5){throw error5;}
...
exports.update_state = function(request, response){
  ...
  if(request.file === undefined){
    db.query(`UPDATE team_info SET title=? , admin_id=? WHERE admin_id = ?`, [post.team_title,  post.admin_id, request.session.uid ], function(error1, result){  if(error1){throw error1;}
      db.query(`UPDATE admin SET id=? ,password=? WHERE id=?`,[post.admin_id,post.admin_pwd ,request.session.uid],function(error2, result2){  if(error2){throw error2;}

        if(post.admin_id !== request.session.uid){
          db.query(`UPDATE activity SET admin_id=? WHERE admin_id=?`,[post.admin_id ,request.session.uid],function(error3, result3){  if(error3){throw error3;}
            db.query(`UPDATE cc SET admin_id=? WHERE admin_id=?`,[post.admin_id ,request.session.uid],function(error4, result4){  if(error4){throw error4;}
              db.query(`UPDATE url SET admin_id=? WHERE admin_id=?`,[post.admin_id ,request.session.uid],function(error5, result5){  if(error5){throw error5;}
    ...
    db.query(`UPDATE team_info SET title=? , admin_id=?, background_image=?, background_mimetype=? WHERE admin_id =?`, [post.team_title,  post.admin_id, base64image,request.file.mimetype ,request.session.uid ], function(error, result){  if(error1){throw error1;}
      db.query(`UPDATE admin SET id=? ,password=? WHERE id=?`,[post.admin_id,post.admin_pwd ,request.session.uid],function(error2, result2){  if(error2){throw error2;}
        if(post.admin_id !== request.session.uid){
          db.query(`UPDATE activity SET admin_id=? WHERE admin_id=?`,[post.admin_id ,request.session.uid],function(error3, result3){  if(error3){throw error3;}
            db.query(`UPDATE cc SET admin_id=? WHERE admin_id=?`,[post.admin_id ,request.session.uid],function(error4, result4){  if(error4){throw error4;}
      ...
      db.query(`DELETE FROM activity WHERE admin_id=? AND  id=?`, [request.session.uid,post.id],function(error2, reuslt){  if(error2){throw error2;}
        db.query(`SELECT * FROM activity WHERE admin_id=? AND score=?`,[request.session.uid,score],function(error3,same_score){  if(error3){throw error3;}
          ...
          if(same_score.length === 0){
            db.query(`DELETE FROM color WHERE admin_id=? AND score=?`,[request.session.uid, score],function(error4, result2){  if(error4){throw error4;}
              response.writeHead(302, {Location: `/admin_main`});
              response.end();
            });
          }else{
